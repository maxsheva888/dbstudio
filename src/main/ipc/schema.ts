...
export function registerSchemaHandlers(): void {
  ipcMain.handle('schema:connect', async (_e, connectionId: string) => {
    ...
    logEntry({ sql: 'SHOW DATABASES', connectionId, database: null, durationMs: Date.now() - ranAt, error: null, rowCount: databases.length, ranAt, kind: 'CONNECT', status: 'ok', sourceLabel: 'system', user: null, tx: false, grade: '?', hints: [] })
    return databases
  })
  ...
  ipcMain.handle('schema:tables', async (_e, connectionId: string, database: string) => {
    ...
    logEntry({ sql: `SHOW TABLE STATUS FROM \`${database}\``, connectionId, database, durationMs: Date.now() - ranAt, error: null, rowCount: tables.length, ranAt, kind: 'OTHER', status: 'ok', sourceLabel: 'system', user: null, tx: false, grade: '?', hints: [] })
    return tables
  })
  ...
  ipcMain.handle('schema:columns', async (_e, connectionId: string, database: string, table: string) => {
    ...
    logEntry({ sql: `SHOW COLUMNS FROM \`${database}\`.\`${table}\``, connectionId, database, durationMs: Date.now() - ranAt, error: null, rowCount: columns.length, ranAt, kind: 'OTHER', status: 'ok', sourceLabel: 'system', user: null, tx: false, grade: '?', hints: [] })
    return columns
  })
